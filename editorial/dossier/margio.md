# Dossier — Margio

<!--
  Compilato dal numero 1 (speciale insediamento, 20 agosto 2026). Ogni riga di
  questo file risale a `data/2026-27/league.json`, `data/2026-27/rosters.json` o
  a `content/2026-27/issue-001/`. Si aggiorna nello stesso commit del numero.
-->

## Anagrafica fantacalcistica

| Campo | Valore |
|---|---|
| **Squadra** | Atletico Piedini |
| **Presidente** | Margio |
| **`teamId`** | `5620502` |
| **Motto ufficiale** | nessuno agli atti |
| **Motto attribuito da questa testata** | «Il portiere è un dettaglio contabile.» |
| **Prima comparsa** | numero 1 |

## Soprannomi

| Soprannome | Origine | Numero |
|---|---|---|
| **il Contabile della Porta** | otto crediti spesi in totale per i tre portieri: due per Desplanches, cinque per Corvi, uno per Palmisani | 1 |
| **l'Uomo che Aveva Ragione** | alla giornata 2 la porta da un credito, Palmisani, fa 7 e Corvi finisce in panchina a 4: coefficiente della porta 0,14, il migliore della lega di due ordini di grandezza | 4 |

## Running joke attivi

### Gli otto crediti della porta

- **Nato nel numero:** 1
- **Stato:** attivo
- **Il fatto d'origine:** asta 2026-27: reparto portieri completo costruito con 8 crediti su 797 spesi, l'1% del budget (`rosters.json`)
- **Come si usa:** si usa come metro di paragone per qualunque spesa: «vale quattro porte dell'Atletico Piedini»
- **Ultimo utilizzo:** numero 4
- **Note:** rinnovato dalla giornata 2, che lo ribalta a suo favore: Palmisani, un credito, fa 7; Corvi, cinque crediti, fa 4 dalla panchina. Il coefficiente della porta dell'Atletico Piedini vale 0,14 crediti per fantapunto contro i 31,0 dell'Amaro Luciano.
- **Note:** nasce con il numero 1; alla quarta ripresa consecutiva passa in
  osservazione e va rinnovato da un dato nuovo (style-guide §10).
### I trecento crediti in panchina

- **Nato nel numero:** 3
- **Stato:** attivo
- **Il fatto d'origine:** giornata 1: Thuram, 300 crediti (37% della spesa), in panchina senza voto; l'Atletico Piedini fa 77,5, il miglior punteggio tra i non vincitori, con Esposito F.P. (31) a 11 e Raspadori (64) a 10; Corvi (5 crediti) a 4,5 (`matchday-01/lineups.json`)
- **Come si usa:** «ha avuto ragione a lasciarlo fuori, e il giornale lo trova sospetto»; si riapre a ogni giornata in cui Thuram gioca o non gioca
- **Ultimo utilizzo:** numero 4
- **Note:** rinnovato e ribaltato alla giornata 2: Thuram viene schierato e chiude a 5,5, il fantavoto più basso dei suoi titolari alla pari con Raspadori. I 300 crediti sono passati dalla panchina al campo senza cambiare esito. Vale La Mano de Dios del Mercato del numero 4.

### Raimondo, quattro crediti, quattordici e mezzo

- **Nato nel numero:** 4
- **Stato:** attivo
- **Il fatto d'origine:** giornata 2: Raimondo, pagato 4 crediti, fa 14,5 in panchina — il fantavoto più alto dell'intera giornata alla pari con Malen, che ne è costati 296. Colpani, 6 crediti, fa 9,5, sempre seduto. Delta panchina-titolare 9,0 (`matchday-02/lineups.json`)
- **Come si usa:** Raimondo è l'unità di conversione tra credito e fantapunto in questa lega: «vale settantaquattro Raimondo»
- **Ultimo utilizzo:** numero 4

## Premi vinti

| Premio | Numero | Motivazione |
|---|---|---|
| La Mano de Dios del Mercato | 4 | Thuram, 300 crediti e colpo celebrato nei numeri 1 e 2, chiude a 5,5 il fantavoto più basso tra i titolari dell'Atletico Piedini alla pari con Raspadori, mentre Raimondo da 4 crediti fa 14,5 in panchina. |

## Precedenti notevoli

| Giornata | Il fatto | Fonte |
|---|---|---|
| Asta 2026-27 | Otto crediti per l'intero reparto portieri, il più basso della lega | `rosters.json` |
| Asta 2026-27 | Otto crediti anche per il solo Mangas, difensore: la porta vale un uomo | `rosters.json` |
| Asta 2026-27 | Thuram pagato 300 crediti, quotazione 29, valore a coefficiente 102 | `rosters.json` |
| Asta 2026-27 | 213 punti di quotazione acquistati, penultimo totale della lega, con 203 crediti lasciati in cassa | `rosters.json`, `league.json` |
| Bombe di Tancredi Soffiata | Thuram per Svilar, pari e patta: cede il 37% della spesa per un portiere che vale 12,5 volte la sua porta da 8 crediti — scambio inventato, cifre vere; stato `lanciata` in `editorial/bombe.json` | `content/2026-27/issue-001/bombe.md` |
| Giornata 1 | Pareggio 2-2 con il J medical, 77,5 a 74,5: il miglior punteggio tra i non vincitori; terzo in classifica | `matchday-01/results.json`, `standings.json` |
| Giornata 1 | Thuram, 300 crediti, in panchina senza voto; Esposito F.P. 11 e Raspadori 10; Corvi, la porta da 5 crediti, 4,5, peggior fantavoto della squadra | `matchday-01/lineups.json` |
| Giornata 1 | Lettera «Un Contabile Insonne» alla Posta del Cuore e segno d'oroscopo nel numero 3 | `content/2026-27/issue-003/rubrica-fissa.md` |
| Giornata 1 | Bomba: riceve Svilar (100) per Corvi (5) più 95 crediti — scambio inventato, cifre vere; stato `lanciata` | `content/2026-27/issue-003/bombe.md` |
| Giornata 2 | Vittoria 2-1 sul Nostalgia Nera, 77 a 68,5; terzo con 4 punti e 154,5 fantapunti | `matchday-02/results.json`, `standings.json` |
| Giornata 2 | Thuram, 300 crediti, finalmente titolare: 5,5, il fantavoto più basso dei suoi alla pari con Raspadori. Calhanoglu (160) 11 | `matchday-02/lineups.json` |
| Giornata 2 | Palmisani, un credito, titolare in porta a 7; Corvi, 5 crediti, in panchina a 4. Coefficiente della porta 0,14, il migliore della lega | `matchday-02/lineups.json`, `content/2026-27/issue-004/classifiche.md` |
| Giornata 2 | Raimondo (4 crediti) 14,5 e Colpani (6) 9,5 in panchina: delta panchina-titolare 9,0, il più alto della giornata alla pari con l'Amaro Luciano; panchina di 11 uomini su 14, tre caselle vuote | `matchday-02/lineups.json` |
| Giornata 2 | Bombe: cede Raimondo al Deportivo La Carogna incassando Berardi (82) più 100 crediti, e riceve Provedel (55) dal J medical cedendo Corvi più 50 — scambi inventati, cifre vere; stato `lanciata` | `content/2026-27/issue-004/bombe.md` |

## Materiale inutilizzato

- Quattro giocatori del Frosinone e quattro della Lazio su venticinque — `rosters.json` — *ancora buono per:* una pagella sulla concentrazione di club.
- 38 crediti in difesa, penultima spesa difensiva della lega dopo l'Aston pirla — `rosters.json` — *ancora buono per:* un'inchiesta tattica.
- 421 crediti su 797 (53%) in attacco, di cui 300 su un solo uomo — `rosters.json` — *ancora buono per:* uno Sconfitto della Settimana, se quell'uomo salta una domenica.
- Panchina di 11 uomini su 14 alla giornata 2, tre caselle vuote: lo stesso difetto per cui l'Aston pirla viene sfottuto dal numero 3, e a lui non lo ha ancora contestato nessuno — `matchday-02/lineups.json` — *ancora buono per:* una pagella o una posta del cuore.
- La porta da otto crediti ha prodotto il miglior coefficiente della lega alla seconda giornata: il tormentone nato per deriderlo si sta trasformando in una tesi — `content/2026-27/issue-004/classifiche.md` — *ancora buono per:* un approfondimento sul reparto portieri.
